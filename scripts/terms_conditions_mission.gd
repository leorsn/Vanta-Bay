...
func _process(_delta: float) -> void:
    ...
    if step == 0 and player.global_position.distance_to(finance_tower_position) <= reach_radius:
        step = 1
        _save_step()
    elif step == 1 and player.global_position.distance_to(contract_drop_position) <= reach_radius:
        _complete()
...
func _complete() -> void:
    ...
    if economy != null:
        economy.award_mission("TERMS & CONDITIONS", reward_cash, reward_rep)
    var saves := get_tree().get_first_node_in_group("story_save_manager") as StorySaveManager
    if saves != null:
