...
const CombatSceneDetailControllerScript = preload("res://scripts/combat_scene_detail_controller.gd")
...
func _ensure_story_runtime() -> void:
    ...
    if get_tree().get_first_node_in_group("combat_scene_detail_controller") == null:
        var combat_scene_detail := CombatSceneDetailControllerScript.new()
        combat_scene_detail.name = "CombatSceneDetailController"
        root.add_child(combat_scene_detail)
    ...
    if get_tree().get_first_node_in_group("environment_detail_controller") == null:
        var environment_detail := EnvironmentDetailControllerScript.new()
        environment_detail.name = "EnvironmentDetailController"
        root.add_child(environment_detail)
    if get_tree().get_first_node_in_group("vehicle_lighting_controller") == null:
    ...
    if get_tree().get_first_node_in_group("facade_window_controller") == null:
        var facade_windows := FacadeWindowControllerScript.new()
        facade_windows.name = "FacadeWindowController"
        root.add_child(facade_windows)
    if get_tree().get_first_node_in_group("ambient_motion_controller") == null:
